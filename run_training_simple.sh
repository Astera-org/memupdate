...
# MemUpdate Training Script - Simplified approach using verl's existing config structure
# Run from verl directory with memupdate data
...
cd /data/users/alan/verl

PROJECT_DIR="$(pwd)"
MEMUPDATE_DIR="/data/users/alan/memupdate"
...
if [ ! -f "$MEMUPDATE_DIR/data/locomo/train.parquet" ]; then
    ...
    cd $MEMUPDATE_DIR && uv run python -m memupdate.data.preprocess_locomo
    cd $PROJECT_DIR
fi
