...
def extract_memories_from_json(file_path: str) -> Dict[str, tuple]:
    """Extract unique memory content from LoCoMo JSON file with conversation IDs.
    
    Returns:
        Dict mapping content -> (sample_id, session, speaker)
    """
    ...
    try:
        ...
        # Map content to its metadata (sample_id, session, speaker)
        memory_map = {}
        
        for conversation in data:
            sample_id = conversation.get('sample_id', 'unknown')
            
            # Only handle LoCoMo format with 'observation' field
                        ...
                        for speaker, speaker_observation in session_obs.items():
                            for fact_entry in speaker_observation:
                                if isinstance(fact_entry, list) and len(fact_entry) >= 2:
                                    ...
                                    if content:
                                        # Store with metadata
                                        memory_map[content] = (sample_id, session_key, speaker)
        
        print(f"✅ Extracted {len(memory_map)} unique memory contents from JSON")
        return memory_map
...
def compute_embeddings_batch(embedding_model, memory_map: Dict[str, tuple], batch_size: int = 32) -> Dict[str, dict]:
    ...
    for i in range(0, len(contents), batch_size):
        ...
        try:
            ...
            # Store in cache with hash keys and metadata
            for content, embedding in zip(batch_contents, batch_embeddings):
                content_hash = hash_content(content)
                sample_id, session, speaker = memory_map[content]
...
def main():
    ...
    parser.add_argument(
        "--input",
        default="/workspace/locomo/data/locomo10.json",
    ...
    if args.input.endswith('.json'):
        memory_map = extract_memories_from_json(args.input)
    else:
    ...
    conv_counts = {}
    for content, (sample_id, _, _) in memory_map.items():
